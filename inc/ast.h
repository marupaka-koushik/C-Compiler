...
using NodeValue = variant<std::monostate, string, int, double, bool, char>;
...
public:
    ...
    ASTNode(NodeType type, NodeValue value = std::monostate()) : type(type), value(value) {
    }
    ...
    string valueToString() const {
        if (holds_alternative<std::monostate>(value))
            return "(null)";
...
ASTNode* createNode(NodeType type, NodeValue value = std::monostate(), Args... children) {
