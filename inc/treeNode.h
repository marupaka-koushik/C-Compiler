...
public:
    ...
    bool isConstant = false;    // For enum constants
    int constantValue = 0;      // Value of enum constant
    ...
    bool isArrayIndex = false; // true if this is an array index expression like arr[i]
    int access = 0;
    vector<int> paramTypes;
    vector<int> paramPointerLevels;  // Pointer levels for each parameter
    vector<int> dimensions;
    vector<pair<string, TreeNode *>> symbolTable;
    string tacResult;
    string basePointer;  // For (*ptr).member, stores the original "ptr" variable
    string incStart;  // For for-loop increment start position
    ...
    BackpatchList returnList;
    BackpatchList caseList;
    BackpatchList defaultList;
